...
void setUpPinModes()
{
  ...
  for (int i = 0; i < motorPins.size(); i++)
  {
    ...
    // ✅ Nueva API LEDC: se configura PWM directamente en el pin
    ledcAttach(motorPins[i].pinEn, PWMFreq, PWMResolution);
  }

  // ✅ Configurar PWM para la luz
  ...
  switch (type) 
  {
    ...
    case WS_EVT_DATA:
    {
      ...
      if (info->final && info->index == 0 && info->len == len && info->opcode == WS_TEXT) 
      {
        ...
        {
          // ✅ Escribir directamente en el pin del motor derecho e izquierdo
          ledcWrite(motorPins[RIGHT_MOTOR].pinEn, valueInt);
          Serial.println(valueInt);
          ledcWrite(motorPins[LEFT_MOTOR].pinEn, valueInt);
        }
        else if (key == "Light")
...
void sendCameraPicture()
{
  ...
  // Esperar a que se libere la cola
  while (true)
  {
  ...
  //Serial.printf("Frame sent in %lu ms\n", millis() - startTime);
}
